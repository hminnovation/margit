...
const getApiKey = () => {
    return new Promise(async (resolve, reject) => {
        ...
        if (fs.existsSync(configPath)) {
            ...
            spinner = ora("Config file does not exist. Prompting user for API key...").start();
            await delay(300); // Add delay
...
const options = yargs(hideBin(process.argv))
    ...
    .command("$0 [message]", "Your message", (yargs) => {
    ...
    .strict()
    .parse();
async function handleArguments(options) {
    if (options.about) {
    ...
    if (!options.dummy && !options.message) {
        console.error("Looks like you forgot to add a message. Run `margit -h` or `margit -help` if you need support");
        process.exit(1);
    }
                ...
                inquirer
                    ...
                    if (answer.runCommand === "Yes") {
                        ...
                        const runCommand = (cmd, index) => {
                            console.log(chalk.inverse("Running " + symbols[index % symbols.length]) +
                                " " +
                                cmd);
